...
export default function AdminScenarios() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'email') {
        // For email type, question field acts as the email body
    ...
    reader.onload = async (event) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          // Simple CSV split (assumes no commas inside values, using | for arrays)
              ...
              <div key={s.id} className="p-3 border border-slate-200 rounded-lg">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">+{s.points} pts | {s.category}</p>
                  </div>
