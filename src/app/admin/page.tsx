  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-sm text-slate-500 font-bold uppercase">Pending Certs</p>
          <p className="text-4xl font-black text-red-600 mt-2">{pendingCerts}</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-sm text-slate-500 font-bold uppercase">Total Points Awarded</p>
          <p className="text-4xl font-black text-blue-600 mt-2">{users.reduce((sum, u) => sum + (u.points || 0), 0)}</p>
        </div>
      </div>
    </div>
  );
