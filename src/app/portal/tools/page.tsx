...
export default function SecurityToolsPage() {
  ...
  const analyzeUrl = () => {
    ...
    try {
      ...
      if (parsed.protocol !== "https:") { score -= 40; indicators.push({type: "red", text: "Insecure HTTP connection (No SSL)"}); } 
      else { indicators.push({type: "green", text: "Secure HTTPS connection detected"}); }
      if (/^\d{1,3}(\.\d{1,3}){3}$/.test(domain)) { score -= 30; indicators.push({type: "red", text: "Uses a raw IP address instead of a domain name"}); }
      if (url.includes("@")) { score -= 20; indicators.push({type: "red", text: "Contains an '@' symbol, often used to hide destinations"}); }
      const fakeDomains = ["paypa1", "g00gle", "arnaz0n", "faceb00k"];
      if (fakeDomains.some(d => domain.includes(d))) { score -= 50; indicators.push({type: "red", text: "Possible brand impersonation (Typosquatting)"}); }
      if (parsed.pathname.includes("login") || parsed.pathname.includes("account")) { indicators.push({type: "amber", text: "Login or account page detected"}); }

      let risk = "LOW RISK - LIKELY SAFE", color = "text-green-600", recommendation = "This website appears safe to interact with.";
      if (score < 80 && score >= 50) { risk = "REVIEW REQUIRED"; color = "text-amber-600"; recommendation = "Proceed with caution. Do not enter sensitive information without verifying the source."; } 
      else if (score < 50) { risk = "HIGH RISK - MALICIOUS"; color = "text-red-600"; recommendation = "Do not provide passwords, payment details, or personal information on this website."; }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 card-shadow">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Analyze a website before interacting with it.</p>
        <div className="flex gap-2">
          <input type="text" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} placeholder="https://example-login.com/account" className="flex-1 px-4 py-2 border border-slate-300 dark:border-slate-600 dark:bg-slate-900 rounded-lg focus:outline-none focus:border-blue-500 font-mono text-sm" />
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 card-shadow">
        ...
        <input type="text" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Type a password to test..." className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 dark:bg-slate-900 rounded-lg focus:outline-none focus:border-blue-500 mb-4 font-mono text-sm" />
